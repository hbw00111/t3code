...
lifecycleLayer("CodexAdapterLive lifecycle", (it) => {
  it.effect("maps native thread goal update and clear notifications", () =>
    Effect.gen(function* () {
      ...
      NodeAssert.equal(events[0]?.type, "thread.goal.updated");
      if (events[0]?.type === "thread.goal.updated") {
        NodeAssert.deepStrictEqual(events[0].payload.goal, {
          objective: "Ship goal projections",
          status: "active",
          tokensUsed: 12,
          timeUsedSeconds: 3,
      ...
      NodeAssert.equal(events[1]?.type, "thread.goal.cleared");
    }),
  );
